...
document.addEventListener("DOMContentLoaded", () => {
  ...
  let animationSpeed = 1.0; // Default speed multiplier
  ...
  const charPatterns = new Array(100)
    .fill(0)
  ...
  function maintainDropDensity() {
    ...
    if (currentDensity < minActiveDensity || activeCount < 5) {
      ...
      for (
        let i = 0;
        i < drops.length && activeCount < minActiveDensity * columns;
        i++
      ) {
  ...
  function generateWave() {
    ...
    if (now - lastWaveTime > waveCooldown) {
      ...
      for (let i = 0; i < waveSize; i++) {
        ...
        if (
          drops[colIndex].y > canvas.height / 2 ||
          drops[colIndex].y > canvas.height
        ) {
  ...
  function getCharColor(j, age, hue = 0) {
    ...
    return `rgba(${Math.floor(r * brightness)}, ${Math.floor(
      g * brightness
    )}, ${Math.floor(b * brightness)}, ${opacity})`;
  }
  ...
  function draw(timestamp) {
    ...
    for (let i = 0; i < drops.length; i++) {
      ...
      if (drop.y > -fontSize && drop.y < canvas.height + fontSize) {
        ...
        for (let j = 0; j < drop.chars.length; j++) {
          ...
          if (charY > -fontSize && charY < canvas.height) {
            ...
            // Age the character (affected by speed)
            drop.chars[j].age += 0.2 * animationSpeed;
      ...
      // Update drop position (affected by speed)
      ...
      if (
        drop.y >= 0 &&
        (drop.chars.length === 0 ||
          drop.y - (drop.chars.length - 1) * fontSize >= fontSize)
      ) {
        drop.chars.unshift({
          value: getRandomChar(),
          age: 0,
      ...
      if (
        (drop.y > canvas.height + 100 && drop.chars.length === 0) ||
  ...
  function createControls() {
    ...
    controlsContainer.innerHTML = `
      ...
      <div class="controls-content">
        <div class="controls-section">
          <div class="controls-header">Speed Control</div>
    ...
    setupSpeedControls(controlsContainer);
  ...
  function setupPanelBehavior(container) {
    ...
    const isPanelCollapsed =
      localStorage.getItem("matrixPanelCollapsed") === "true";
    const panelPosition = JSON.parse(
      localStorage.getItem("matrixPanelPosition") ||
    ...
    if (isTouchDevice) {
      mainHeader.addEventListener("touchstart", startDragTouch, {
        passive: false,
      });
    } else {
  ...
  function setupOverlayControls(container) {
    ...
    const savedShowOverlay =
      localStorage.getItem("matrixShowOverlay") === "true";
    const savedOverlaySize =
      localStorage.getItem("matrixOverlaySize") || "medium";
    const savedOverlayPosition =
      localStorage.getItem("matrixOverlayPosition") || "center";
  ...
  // Setup speed controls
  function setupSpeedControls(container) {
    ...
    const savedSpeed = localStorage.getItem("matrixSpeed") || "1.0";
    ...
    speedSlider.addEventListener("input", (e) => {
      ...
      speedValue.textContent = `${animationSpeed.toFixed(1)}x`;
      localStorage.setItem("matrixSpeed", animationSpeed.toString());
    });
  }
